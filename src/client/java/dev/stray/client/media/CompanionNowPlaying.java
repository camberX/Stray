...
final class CompanionNowPlaying {
	private static final HttpClient CLIENT = HttpClient.newBuilder()
		...
		.build();
	private static final String AUTH_ID = "stray";
	...
	String statusHint() {
		if (awaitingAuth) {
			return "Allow Stray in YouTube Music";
		}
		if (reachable) {
	...
	private void authorize(HttpRequest.Builder builder) {
		String token = StrayConfig.get().musicApiToken;
	...
	private void askAuth(String origin) {
		...
		long now = System.currentTimeMillis();
		if (now - lastAuthAskMs < AUTH_COOLDOWN_MS && (StrayConfig.get().musicApiToken == null || StrayConfig.get().musicApiToken.isBlank())) {
			return;
		}
		lastAuthAskMs = now;
		authInFlight = true;
	...
	private static List<String> endpoints() {
		...
		int custom = StrayConfig.get().musicApiPort;
