...
/**
 * Stray-styled player nametags that keep drawing past vanilla's 64-block cutoff.
 * Tags scale with camera distance, then by the Size slider. Style picks Stray
...
public final class NametagRenderer {
	...
	private static final String BADGE_BRAND = "STRAY";
	...
	private static Component label(AbstractClientPlayer player) {
		if (player == Minecraft.getInstance().player && StrayConfig.get().nickEnabled) {
