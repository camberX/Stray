...
public final class HudLayout {
	...
	private static Box computeBox(Id id, Font font, int guiW, int guiH) {
		...
		StrayConfig.HudSlot slot = slot(id);
	...
	public static float scale(Id id) {
		...
		StrayConfig.HudSlot slot = slot(id);
		if (slot != null) {
			return StrayConfig.clampHudScale(slot.scale);
		}
		return switch (id) {
			case WATERMARK -> StrayConfig.clampHudScale(config.hudWatermarkScale);
	...
	public static void setScale(Id id, float scale) {
		boxTick = Integer.MIN_VALUE;
		float value = StrayConfig.clampHudScale(scale);
		StrayConfig config = StrayConfig.get();
		StrayConfig.HudSlot slot = slot(id);
		if (slot != null) {
			slot.scale = value;
	...
	private static StrayConfig.HudSlot slot(Id id) {
		StrayConfig config = StrayConfig.get();
		return switch (id) {
