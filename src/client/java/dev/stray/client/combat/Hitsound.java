...
public final class Hitsound {
	private static final SoundEvent SOUND = SoundEvent.createVariableRangeEvent(Stray.id("hit"));
	...
	private static boolean wantArrow(StrayConfig config) {
	...
	private static void land(StrayConfig config, boolean sound, boolean mark) {
		if (sound) {
			play(config);
		}
		if (mark) {
	...
	private static void play(StrayConfig config) {
		Minecraft client = Minecraft.getInstance();
		if (client == null) {
			return;
		}
