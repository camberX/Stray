...
import dev.stray.client.ui.CapeCreatorScreen;
...
public final class CustomCape {
	...
	private static final Identifier TEXTURE_ID = Stray.id("cape");
		...
		Thread thread = new Thread(() -> {
			...
			try {
				try {
					...
					Stray.LOGGER.warn("Native cape picker failed, trying Explorer", exception);
					try {
						selected = awtPick();
					} catch (Throwable explorer) {
						Stray.LOGGER.warn("Explorer cape picker failed", explorer);
	...
	private static void install(
		...
		Minecraft.getInstance().execute(() -> {
			...
			try {
				if (croppedImage) {
					Path stored = FabricLoader.getInstance().getConfigDir().resolve("stray-cape.png");
				...
				DynamicTexture texture = new DynamicTexture(() -> "stray-cape", atlas);
	...
	private static byte[] encodePng(NativeImage image) throws java.io.IOException {
		Path tmp = Files.createTempFile("stray-cape", ".png");
		try {
