...
/**
 * Shared cape shop. Everyone with Stray fetches {@code /capes/{uuid}.png}
...
public final class ShopCape {
	private static final String SHOP_URL = "https://stray.gay";
	...
	private static void register(UUID uuid, Slot slot, byte[] bytes, String hash) {
		...
		Minecraft.getInstance().execute(() -> {
			try {
				Identifier id = Stray.id("shop_cape/" + uuid.toString().replace("-", "") + "_" + safeHash);
				DynamicTexture texture = new DynamicTexture(() -> "stray-shop-cape-" + uuid, atlas);
