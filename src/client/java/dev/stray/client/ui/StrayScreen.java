...
public class StrayScreen extends Screen {
	...
	private static final SearchEntry[] SEARCH = {
		...
		new SearchEntry("Backwards walk", Tab.CAMERA, "Camera"),
		new SearchEntry("1.8 backwards walk", Tab.CAMERA, "Camera"),
		...
		new SearchEntry("Party stats", Tab.ASSIST, "Assist"),
		...
		new SearchEntry("All tools", Tab.HOLLOWS, "Nucleus"),
	...
	private void drawColumns(GuiGraphicsExtractor graphics, Font font, int mouseX, int mouseY) {
		...
		switch (tab) {
			case WORLD -> {
				...
				y = featureCard(graphics, font, right, top, col, cardHeight(5), "Camera");
				...
				y = toggle(graphics, font, rx, y, iw, mouseX, mouseY, "1.8 skull hold", config.legacySkullHold, v -> config.legacySkullHold = v);
				toggle(graphics, font, rx, y, iw, mouseX, mouseY, "Backwards walk", config.legacyBackwardsWalk, v -> config.legacyBackwardsWalk = v);
			}
			case COMBAT -> {
				...
				float partyY = rogueY + cardHeight(1) + 8;
				y = featureCard(graphics, font, left, partyY, col, cardHeight(1), "Party stats");
				toggle(graphics, font, ix, y, iw, mouseX, mouseY, "Join messages", config.partyFinderStats, v -> config.partyFinderStats = v);
	...
	private void drawControlColumns(
		...
		switch (tab) {
			...
			case CAMERA -> {
				...
				y = toggleCard(graphics, font, left, y, col, mouseX, mouseY, "Backwards walk", config.legacyBackwardsWalk, v -> config.legacyBackwardsWalk = v);
			...
			case ASSIST -> {
				...
				y = toggle(graphics, font, ix, y, iw, mouseX, mouseY, "Players", config.triggerbotPlayers, v -> config.triggerbotPlayers = v);
				slider(graphics, font, ix, y, iw, "Humanize", Math.round(config.triggerbotHumanize * 100) + "%", config.triggerbotHumanize, v -> config.triggerbotHumanize = StrayConfig.clamp(v, 0f, 1f));
				float rogueY = toggleCard(graphics, font, left, aim + triggerH + 8, col, mouseX, mouseY, "Auto rogue sword", config.autoRogueEnabled, v -> config.autoRogueEnabled = v);
				toggleCard(graphics, font, left, rogueY, col, mouseX, mouseY, "Party stats", config.partyFinderStats, v -> config.partyFinderStats = v);
	...
	private void drawFeatureFields(
		...
		switch (feature) {
			...
			case PATHS -> {
				...
				String recLabel = rec
					? "Stop recording (" + PathRecorder.liveRecording().size() + " pts)"
					: "Record on " + clip(font, PathRecorder.islandLabel(), (int) iw - 80);
				y = clickRow(graphics, font, ix, y, iw, mouseX, mouseY, recLabel, PathRecorder::toggle);
				y = bindRow(graphics, font, ix, y, iw, mouseX, mouseY, "Record key", 9, OdinClicks.parseKey(config.pathRecordKey));
				y = toggle(graphics, font, ix, y, iw, mouseX, mouseY, "Through walls", config.pathThroughWalls, v -> config.pathThroughWalls = v);
				y = slider(graphics, font, ix, y, iw, "Width", String.format(java.util.Locale.ROOT, "%.1f", config.pathWidth), (config.pathWidth - 1f) / 5f, v -> config.pathWidth = StrayConfig.clamp(1f + v * 5f, 1f, 6f));
				y = colorRow(graphics, font, ix, y, iw, mouseX, mouseY, "Color", config.pathRgb, PickerTarget.PATHS);
				List<PathRecorder.Recording> here = PathRecorder.here();
				int shown = Math.min(here.size(), 10);
				for (int i = 0; i < shown; i++) {
					PathRecorder.Recording recording = here.get(i);
					String label = clip(font, recording.name, (int) iw - 70) + "  " + Math.round(PathRecorder.length(recording)) + "m";
					y = toggle(graphics, font, ix, y, iw, mouseX, mouseY, label, recording.visible, v -> PathRecorder.setVisible(recording, v));
				}
			}
			case NUCLEUS -> {
				y = toggle(graphics, font, ix, y, iw, mouseX, mouseY, "Robot parts", config.nucleusAlertParts, v -> config.nucleusAlertParts = v);
				toggle(graphics, font, ix, y, iw, mouseX, mouseY, "Divan tools", config.nucleusAlertTools, v -> config.nucleusAlertTools = v);
			}
			case NODE_ESP -> {
