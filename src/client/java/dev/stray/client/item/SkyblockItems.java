...
public final class SkyblockItems {
	...
	public static void load() {
		...
		loaded = true;
		try (InputStream raw = SkyblockItems.class.getResourceAsStream("/assets/stray/skyblock_items.tsv.gz")) {
			if (raw == null) {
				Stray.LOGGER.warn("Skyblock item catalog is missing");
				return;
			}
			...
			Stray.LOGGER.info("Loaded {} Skyblock item ids", BY_ID.size());
		} catch (Exception exception) {
			Stray.LOGGER.warn("Could not read Skyblock item catalog", exception);
		}
	}
