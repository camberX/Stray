...
Stray includes Ender Node highlighting, mining and farming tools, configurable HUD elements, custom menus, entity ESP, capes, media controls, and a compact in-game settings interface.
...
3. Download Stray from [stray.gay](https://stray.gay) and place the JAR in the same folder.
...
- Right Shift is the default keybind (Controls → Stray). Press it again to close (the menu eases out).
- `/stray toggle` or `/st toggle` flips node markers without opening the menu.
- `/st farmkeys` (or `/st fk`) swaps the Attack/Destroy and Jump bindings, makes the swapped Attack/Destroy key toggle held attack, and lowers sensitivity to Minecraft's minimum. Run it again to restore your exact bindings and sensitivity.
...
The default menu key is **Right Shift**. Keybinds can be changed under **Controls → Stray**.

| Command | Description |
| --- | --- |
| `/stray`, `/st`, or `/vm` | Open the Stray menu |
| `/stray toggle` or `/st toggle` | Toggle Ender Node markers |
| `/st edit` | Open the held-item appearance editor |
| `/st loadouts` | Open the custom Loadouts menu |
| `/st wardrobe` | Open the custom Wardrobe menu |
| `/st rawmats [item]` | Track raw materials for a SkyBlock item |
| `/st rawmats refresh` | Refresh profile storage data |
| `/st rawmats clear` | Hide the material tracker |
| `/st esp <name>` | Add a nametag ESP filter |
| `/st esp clear [name]` | Remove nametag ESP filters |
| `/st farmkeys` or `/st fk` | Toggle farming controls |
| `/st music` | Show music integration status |
...
The `web` directory contains the Stray download site and cape service. Local development can be started with:
...
Stray is released under the [CC0 1.0 Universal](LICENSE) public-domain dedication.
